...
func main() {
	...
	maxMemory := flag.Int("max-memory", 1, "max memory per CPU-core in MBs")
	maxGoRoutines := flag.Int("max-goroutines", 100000, "max number of goroutines per quicksort")
	noTapeCleanup := flag.Bool("no-tape-cleanup", false, "do not remove the .tapes/ directory")
	check := flag.Bool("check", false, "check if index file is sorted correctly")
	verbose := flag.Bool("verbose", false, "display index entries when checking sort")
	flag.Parse()
